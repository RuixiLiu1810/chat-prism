...
fn copy_file_to_build(src: &Path, dst: &Path) -> std::io::Result<()> {
    if let Some(parent) = dst.parent() {
        std::fs::create_dir_all(parent)?;
    }

    let copy_once = || std::fs::copy(src, dst).map(|_| ());
    ...
    if let Err(err) = first {
        ...
        // Self-heal for stale read-only files inside .prism/build.
        if dst.exists() {
            let _ = ensure_owner_writable(dst);
        }
        if let Some(parent) = dst.parent() {
            let _ = ensure_owner_writable(parent);
        }

        if let Err(retry_err) = copy_once() {
            if retry_err.kind() == std::io::ErrorKind::PermissionDenied && dst.exists() {
                let _ = ensure_owner_writable(dst);
                let _ = std::fs::remove_file(dst);
                copy_once().map_err(|final_err| {
                    std::io::Error::new(
                        final_err.kind(),
                        format!(
                            "copy {} -> {} failed after permission self-heal: {}",
                            src.display(),
                            dst.display(),
                            final_err
                        ),
                ...
                return Err(std::io::Error::new(
                    retry_err.kind(),
                    format!(
                        "copy {} -> {} failed after permission retry: {}",
                        src.display(),
                        dst.display(),
                        retry_err
                    ),
    ...
    // Copying preserves source mode; force owner-write on build outputs
    // so future sync passes can overwrite safely.
    ensure_owner_writable(dst).map_err(|err| {
        std::io::Error::new(
            err.kind(),
            format!("set writable {} failed: {}", dst.display(), err),
        )
    })?;
    Ok(())
}

fn write_file_to_build(dst: &Path, data: &[u8]) -> std::io::Result<()> {
    if let Some(parent) = dst.parent() {
        std::fs::create_dir_all(parent)?;
    }

    let write_once = || std::fs::write(dst, data);
    let first = write_once();
    if let Err(err) = first {
        if err.kind() != std::io::ErrorKind::PermissionDenied {
            return Err(std::io::Error::new(
                err.kind(),
                format!("write {} failed: {}", dst.display(), err),
            ));
        }
        ...
        write_once().map_err(|retry_err| {
            std::io::Error::new(
                retry_err.kind(),
                format!(
                    "write {} failed after permission self-heal: {}",
                    dst.display(),
                    retry_err
                ),
            )
...
fn copy_dir_recursive(src: &Path, dst: &Path) -> std::io::Result<()> {
    ...
    for entry in std::fs::read_dir(src)? {
        ...
        if src_path.is_dir() {
            ...
            copy_file_to_build(&src_path, &dst_path)?;
        }
    }
    Ok(())
}
        ...
        if src_path.is_dir() {
            ...
            if !is_artifact && !is_synctex {
                ...
                if metadata.len() == 0 {
                    ...
                    if !data.is_empty() {
                        write_file_to_build(&dst_path, &data)?;
                    } else {
                        copy_file_to_build(&src_path, &dst_path)?;
                    }
                } else {
...
";
    ...
    #[test]
    fn test_sync_source_files_self_heals_read_only_destination_file() {
        let src = tempfile::tempdir().unwrap();
        let dst = tempfile::tempdir().unwrap();

        let src_file = src.path().join("main.tex");
        let dst_file = dst.path().join("main.tex");
        std::fs::write(&src_file, "new content").unwrap();
        std::fs::write(&dst_file, "old content").unwrap();
        ...
        assert_eq!(std::fs::read_to_string(&dst_file).unwrap(), "new content");

        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
            let perms = std::fs::metadata(&dst_file).unwrap().permissions();
            assert_ne!(perms.mode() & 0o200, 0);
        }
