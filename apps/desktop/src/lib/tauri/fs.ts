...
const IGNORED_EXTENSIONS = new Set([
  // Ignored file extensions: LaTeX build artifacts
  ".aux",
  ".log",
  ".out",
  ".toc",
...
function getFileType(name: string): ProjectFileType | null {
  ...
  // Skip ignored file extensions (LaTeX build artifacts only)
...
export async function appendJsonLineToProject(
  rootPath: string,
  relativePath: string,
  payload: unknown,
): Promise<string> {
  const fullPath = await join(rootPath, relativePath);
  const lastSep = Math.max(
    fullPath.lastIndexOf("/"),
    fullPath.lastIndexOf("\\"),
  );
  ...
  if (await exists(fullPath)) {
    previous = await readTextFile(fullPath);
  }
  ...
  await writeTextFile(fullPath, next);
  return fullPath;
}
...
export async function copyFileToProject(
  ...
  // If source and requested target are the same file, treat as no-op.
  // This prevents creating " (1)" duplicates when re-attaching a file
  // that is already in the target location.
  const requestedPath = await join(rootPath, targetName);
  const normalizePath = (p: string) =>
    p.replace(/\\/g, "/").replace(/\/+$/, "");
  if (normalizePath(sourcePath) === normalizePath(requestedPath)) {
    return targetName;
  }
