...
let _minLogLevel = parseLogLevel(localStorage.getItem("logLevel"));
window.addEventListener("storage", (e) => {
  if (e.key === "debug") _debugEnabled = !!e.newValue;
  if (e.key === "logLevel") _minLogLevel = parseLogLevel(e.newValue);
});
...
export const useLogStore = create<LogStore>((set) => ({
  ...
  log: (level, source, message, data) => {
    ...
    if (LOG_LEVEL_ORDER[level] < LOG_LEVEL_ORDER[_minLogLevel]) return;
