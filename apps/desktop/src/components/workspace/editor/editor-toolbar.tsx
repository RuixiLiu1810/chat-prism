...
export function EditorToolbar({
  ...
  const preferredEditor = useSettingsStore(
    (s) => s.effective.general.openInEditor.defaultEditor,
  );
  ...
  const orderedEditors = useMemo(() => {
    if (preferredEditor === "system") return editors;
    const index = editors.findIndex((e) => e.id === preferredEditor);
    if (index <= 0) return editors;
    return [editors[index], ...editors.slice(0, index), ...editors.slice(index + 1)];
  }, [editors, preferredEditor]);

  const primaryEditor = orderedEditors[0];
  const showQuickOpen =
    !!primaryEditor &&
    (orderedEditors.length === 1 || preferredEditor !== "system");
    ...
    return (
      <div className="flex h-[calc(36px+var(--titlebar-height))] items-center justify-between border-border border-b bg-muted/30 px-2 pt-[var(--titlebar-height)]">
        ...
        <div className="flex items-center gap-0.5">
          ...
          {showQuickOpen && primaryEditor && (
            <TooltipIconButton
              tooltip={`Open in ${primaryEditor.name}`}
              onClick={() => openInEditor(primaryEditor.id)}
            >
          ...
          {orderedEditors.length > 1 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                {orderedEditors.map((editor, index) => (
                  <DropdownMenuItem
                    key={editor.id}
                    onClick={() => openInEditor(editor.id)}
                  >
                    {editor.name}
                    {index === 0 && preferredEditor !== "system" ? " (Default)" : ""}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
