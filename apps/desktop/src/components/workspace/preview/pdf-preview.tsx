...
const PDF_SELECTION_EDIT_PROFILE: AgentTurnProfile = {
  taskKind: "selection_edit",
  selectionScope: "selected_span",
  responseMode: "reviewable_change",
  samplingProfile: "edit_stable",
  sourceHint: "pdf_selection_action",
};

const PDF_FILE_EDIT_PROFILE: AgentTurnProfile = {
  taskKind: "file_edit",
  selectionScope: "none",
  responseMode: "reviewable_change",
  samplingProfile: "edit_stable",
  sourceHint: "pdf_file_action",
};
  ...
  const handlePdfToolbarSendPrompt = useCallback(
    (prompt: string) => {
      ...
      const sourceFile = resolvedSource
        ? files.find((file) => file.relativePath === resolvedSource.file) ?? null
        : null;
    ...
    [pdfSelection, pdfContextLabel, resolvedSource, buildPdfContext, files],
  );
