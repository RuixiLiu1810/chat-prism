...
const MAX_RENDER_DPI = 216;
const FALLBACK_RENDER_DPI = 144;
...
export const MupdfPage = memo(function MupdfPage({
  ...
  const renderPage = useCallback(() => {
    ...
    const requestedDpi = scale * 72 * dpr;
    const dpi = Math.min(requestedDpi, MAX_RENDER_DPI);

    const drawAtDpi = (targetDpi: number, attempt: "primary" | "fallback") => {
      client
        .drawPage(docId, pageIndex, targetDpi)
        ...
        .catch((err) => {
          ...
          if (
            attempt === "primary" &&
            message.includes("MuPDF worker timeout") &&
            targetDpi > FALLBACK_RENDER_DPI
          ) {
            log.warn(`Render timeout page ${pageIndex}; retrying at lower DPI`, {
              requestedDpi,
              cappedDpi: dpi,
              fallbackDpi: FALLBACK_RENDER_DPI,
            });
            drawAtDpi(FALLBACK_RENDER_DPI, "fallback");
            return;
          }
          log.error(`Render error page ${pageIndex}`, {
            error: message,
            requestedDpi,
            attemptedDpi: targetDpi,
          });
        });
    };

    drawAtDpi(dpi, "primary");
